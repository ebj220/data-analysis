...
%% Load Videos
f1_data = VideoReader('monte_carlo_low.mp4');

vidFrames = read(f1_data);
numFrames = get(f1_data,'numberOfFrames');
for k = 1 : numFrames
...
figure(3)
for j = 1:3
    frame2 = reshape(X_for(:,290 +(j-1)*40),[540 960]);
    frame2 = uint8(140-real(frame2));
    subplot(2,3,j)
    imshow(frame2);
    title(['Frame ', num2str(290+(j-1)*40)])
    axis('equal','tight')
end
for j = 1:3
    frame1 = reshape(X_dmd(:,290 +(j-1)*40),[540 960]);
